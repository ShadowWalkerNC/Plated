...
export function Step4Location() {
  ...
  function patchLocation(patch: Record<string, unknown>) {
    updateSchema({
      locations: [
        {
          ...location,
  ...
  return (
    <section className={styles.step}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>Add your address, contact info, map links, and weekly schedule.</p>
      </header>
      ...
      <div className={styles.grid2}>
        ...
        <Field label="Apple Maps URL">
          <input
            value={location.appleMapsUrl}
            onChange={(e) => patchLocation({ appleMapsUrl: e.target.value })}
      ...
      <Field label="Holiday note" hint="Optional note shown beneath regular hours.">
        <input
          value={location.holidayNote}
          onChange={(e) => patchLocation({ holidayNote: e.target.value })}
      ...
      <div className={styles.hoursSection}>
        ...
        <HoursBuilder
          ...
          onChange={(schedule) => patchLocation({ hours: { ...location.hours, schedule } })}
        />
      </div>
    </section>
  );
