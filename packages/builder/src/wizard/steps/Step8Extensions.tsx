...
import editorStyles from './Step8Extensions.module.css';
...
export function Step8Extensions() {
  ...
  function setPlausible(enabled: boolean) {
    updateSchema({
      extensions: {
        ...
        analytics: {
          ...ext.analytics,
          plausible: { enabled, domain: ext.analytics?.plausible?.domain ?? '' },
        },
      },
    });
  }
  ...
  function setGa4(enabled: boolean) {
    updateSchema({
      extensions: {
        ...
        analytics: {
          ...ext.analytics,
          ga4: { enabled, measurementId: ext.analytics?.ga4?.measurementId ?? '' },
        },
      },
    });
  }
  ...
  function setToggle(
    key: keyof Omit<typeof ext, 'analytics' | 'cdnLibraries' | 'plugins'>,
    enabled: boolean,
  ) {
  ...
  const rows: Array<[
    keyof Omit<typeof ext, 'analytics' | 'cdnLibraries' | 'plugins'>,
    string,
    string,
  ]> = [
    ...
    <section className={styles.step}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Choose optional add-ons. You can change these any time and regenerate the site.
        </p>
      </header>
      ...
      <div className={styles.toggleGrid}>
        <label className={styles.toggleCard}>
          <div className={styles.toggleCardBody}>
            ...
            <div className={styles.toggleDescription}>
              Privacy-first, cookie-free analytics. On by default — no GDPR consent banner needed.
            </div>
          </div>
          <input
            type="checkbox"
            checked={plausibleEnabled}
        ...
        <label className={styles.toggleCard}>
          <div className={styles.toggleCardBody}>
            ...
            <div className={styles.toggleDescription}>
              Optional GA4 measurement ID. Requires a cookie consent banner — enable cookieBanner below.
            </div>
          </div>
          <input
            type="checkbox"
      ...
      {/* ── Editor CTA ─────────────────────────────────────────────── */}
      <div className={editorStyles.editorCta}>
        <div className={editorStyles.ctaText}>
          <span className={editorStyles.ctaIcon}>🧱</span>
          <div>
            <div className={editorStyles.ctaTitle}>Customise your page layout</div>
            <div className={editorStyles.ctaDesc}>
              Drag, reorder, and toggle content blocks on every page before you generate.
            </div>
          </div>
        </div>
        <button
          className={editorStyles.ctaBtn}
          onClick={openEditor}
          type="button"
        >
          Open Block Editor →
