// Shared HTML wrapper used by every email
...
<head>
  ...
  <style>
    ...
    .hdr  { background:#1a1208; padding:28px 36px; display:flex; align-items:center; gap:12px; }
    .logo { font-size:1.5rem; font-weight:800; color:#f4ede4; letter-spacing:-0.02em; }
    .logo span { color:#c98f4a; }
    ...
    .btn { display:inline-block; margin:8px 0 16px; padding:12px 24px;
           background:#8a4b2f; color:#fff8f2; text-decoration:none;
...
<body>
  <div class="wrap">
    <div class="hdr"><div class="logo">Nex<span>CMS</span></div></div>
    <div class="body">${body}</div>
    <div class="ftr">
      &copy; ${new Date().getFullYear()} NexCMS &mdash;
      <a href="https://nexcms.io/unsubscribe">Unsubscribe</a> &middot;
      <a href="https://nexcms.io">nexcms.io</a>
    </div>
  </div>
</body>
</html>`;
...
export function welcomeTemplate({ name, dashboardUrl }: WelcomeData) {
  return {
    subject: 'Welcome to NexCMS 🍷',
    html: emailShell(`
      <h1>Welcome, ${name}!</h1>
      <p>Your NexCMS account is ready. Start by creating your first project — it takes less than two minutes.</p>
      <a class="btn" href="${dashboardUrl}">Open Dashboard</a>
      <hr class="divider" />
      <p class="muted">If you didn't sign up for NexCMS, you can safely ignore this email.</p>
    `),
  };
}
...
export function deploySuccessTemplate({ projectName, provider, deployUrl, filesWritten, dashboardUrl }: DeploySuccessData) {
  return {
    ...
    html: emailShell(`
      ...
      <hr class="divider" />
      <p class="muted">Deployed from your <a href="${dashboardUrl}">NexCMS dashboard</a>.</p>
    `),
  };
}
...
export function deployFailureTemplate({ projectName, provider, errorMessage, dashboardUrl }: DeployFailureData) {
  return {
    ...
    html: emailShell(`
      ...
      <hr class="divider" />
      <p class="muted">If this keeps happening, contact <a href="mailto:support@nexcms.io">support@nexcms.io</a>.</p>
    `),
  };
}

// ─ Billing: upgrade confirmed ──────────────────────────────────────────────
...
export function paymentFailedTemplate({ name, billingUrl }: PaymentFailedData) {
  return {
    ...
    html: emailShell(`
      ...
      <hr class="divider" />
      <p class="muted">If you need help, email <a href="mailto:support@nexcms.io">support@nexcms.io</a>.</p>
    `),
  };
}
