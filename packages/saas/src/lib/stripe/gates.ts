...
export async function canCreateProject(userId: string): Promise<{ allowed: boolean; reason?: string }> {
  ...
  const rows = await db.select({ count: count() }).from(projects).where(eq(projects.userId, userId));
...
export async function canDeploy(userId: string): Promise<{ allowed: boolean; reason?: string }> {
  ...
  const rows = await db.select({ count: count() }).from(deployments)
    .where(and(eq(deployments.userId, userId), gte(deployments.createdAt, startOfMonth)));
  const used = rows[0]?.count ?? 0;
  if (used >= plan.deployLimit) {
    return { allowed: false, reason: `You've used ${used}/${plan.deployLimit} deploys this month on the ${plan.name} plan.` };
  }
  return { allowed: true };
}
...
export async function canUseTheme(userId: string, themeId: string): Promise<{ allowed: boolean; reason?: string }> {
  const { plan } = await getUserPlan(userId);
  if (!(plan.themes as readonly string[]).includes(themeId)) {
    return { allowed: false, reason: `The "${themeId}" theme requires a Pro plan or higher.` };
  }
  return { allowed: true };
}

export async function canUseCustomDomain(userId: string): Promise<{ allowed: boolean; reason?: string }> {
  const { plan } = await getUserPlan(userId);
  if (!plan.customDomain) {
    return { allowed: false, reason: 'Custom domains require a Pro plan or higher.' };
  }
  return { allowed: true };
}
